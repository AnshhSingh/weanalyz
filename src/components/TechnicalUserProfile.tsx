...
import { FaUser, FaTachometerAlt, FaClipboardList, FaChartLine } from "react-icons/fa";
...
function TechnicalUserProfile() {
  const [feedback, setFeedback] = useState("");
  const [rating, setRating] = useState(0);

  return (
    <Layout sidebarItems={technicalSidebar}>
      <h2 className="main-title" style={{ textAlign: 'left', marginBottom: 24 }}>Technical Profile</h2>
      <div style={{ background: '#7be0d6', borderRadius: 0, padding: 32, minHeight: 400, display: 'flex', alignItems: 'flex-start', gap: 48 }}>
        <div style={{ display: 'flex', gap: 48 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 16, boxShadow: '2px 4px 8px #d3d3d3', padding: '32px 32px 32px 32px', minWidth: 300, minHeight: 280, position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <span style={{ position: 'absolute', top: 18, right: 18, cursor: 'pointer' }} onClick={() => window.location.href = '/technical/profile/edit'}>
...
export default TechnicalUserProfile; 